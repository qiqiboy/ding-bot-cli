...
const dingCLI = _commander.default.version(_package.default.version, '-v, --version').description('Send message by DingDing bot').arguments('<type> <jsonBody>').option('-t, --token <token>', '机器人的access token').option('-a, --at [mobiles]', '被@人的手机号（以空格或者半角逗号间隔多个手机号，如果传递 all 表示@全部人）', value => value && value.split(/\s*[\s,|]\s*/g)).action(async (msgType, jsonBody) => {
  ...
  if (!msgTypes.includes(msgType)) {
    return spinner.fail(_chalk.default.red(`<type> must be one of [text, link, markdown, actionCard, feedCard]`));
  }
